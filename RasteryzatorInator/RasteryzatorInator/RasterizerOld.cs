...
internal class RasterizerOld
{
    private Buffer buffer;

    public RasterizerOld(Buffer _buffer)
    {
        buffer = _buffer;
    }
    ...
    public void Triangle(Point3D p1, Point3D p2, Point3D p3, RawColor? defaultColor = null)
    {
        ...
        int screenX1 = (int)((p1.cX + 1) * 0.5f * width);
        int screenY1 = (int)((p1.cY + 1) * 0.5f * height);

        int screenX2 = (int)((p2.cX + 1) * 0.5f * width);
        int screenY2 = (int)((p2.cY + 1) * 0.5f * height);

        int screenX3 = (int)((p3.cX + 1) * 0.5f * width);
        int screenY3 = (int)((p3.cY + 1) * 0.5f * height);
        ...
        if (!fullColorMode)
        {
            for (int y = minY; y <= maxY; y++)
            {
                for (int x = minX; x <= maxX; x++)
                {
                    int a = (screenX1 - screenX2) * (y - screenY1) - (screenY1 - screenY2) * (x - screenX1);
                    int b = (screenX2 - screenX3) * (y - screenY2) - (screenY2 - screenY3) * (x - screenX2);
                    int c = (screenX3 - screenX1) * (y - screenY3) - (screenY3 - screenY1) * (x - screenX3);
                    ...
                    {
                        float denominator = (screenY2 - screenY3) * (screenX1 - screenX3) + (screenX3 - screenX2) * (screenY1 - screenY3);
                        float lambda1 = ((screenY2 - screenY3) * (x - screenX3) + (screenX3 - screenX2) * (y - screenY3)) / denominator;
                        float lambda2 = ((screenY3 - screenY1) * (x - screenX3) + (screenX1 - screenX3) * (y - screenY3)) / denominator;
